...
check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error} Tài khoản hiện tại không phải là ROOT (hoặc không có quyền ROOT), không thể tiếp tục thao tác, vui lòng chuyển sang tài khoản ROOT hoặc sử dụng lệnh ${Green_background_prefix}sudo su${Font_color_suffix} để có quyền ROOT tạm thời (sau khi thực hiện có thể sẽ yêu cầu nhập mật khẩu của tài khoản hiện tại)." && exit 1
}
...
net.ipv4.tcp_congestion_control = bbr" >>/etc/sysctl.d/local.conf && sysctl --system >/dev/null 2>&1
		...
		echo -e "$Error系统内核版本过低，Không thể hỗ trợ TCP Fast Open ！"
	fi
}
...
check_new_ver(){
	...
	[[ -z ${new_ver} ]] && echo -e "${Error} Không thể lấy phiên bản mới nhất của Shadowsocks Rust!" && exit 1
	echo -e "${Info} Đã phát hiện phiên bản mới nhất của Shadowsocks Rust là [ ${new_ver} ]"
}

check_ver_comparison(){
	now_ver=$(cat ${Now_ver_File})
	if [[ "${now_ver}" != "${new_ver}" ]]; then
		echo -e "${Info} Đã phát hiện phiên bản mới của Shadowsocks Rust [ ${new_ver} ], phiên bản hiện tại [ ${now_ver} ]"
...
# 官方源
stable_Download() {
	echo -e "${Info} Mặc định bắt đầu tải Shadowsocks Rust từ nguồn chính thức ……"
	wget --no-check-certificate -N "https://github.com/shadowsocks/shadowsocks-rust/releases/download/${new_ver}/shadowsocks-${new_ver}.${arch}-unknown-linux-gnu.tar.xz"
	if [[ ! -e "shadowsocks-${new_ver}.${arch}-unknown-linux-gnu.tar.xz" ]]; then
		echo -e "${Error} Tải Shadowsocks Rust từ nguồn chính thức thất bại!"
		return 1 && exit 1
	else
		tar -xvf "shadowsocks-${new_ver}.${arch}-unknown-linux-gnu.tar.xz"
	fi
	if [[ ! -e "ssserver" ]]; then
        ...
        echo -e "${Info} Đã tải và cài đặt xong Shadowsocks Rust!"
		return 0
	fi
}
...
backup_Download() {
	...
	if [[ ! -e "shadowsocks-v1.14.1.${arch}-unknown-linux-gnu.tar.xz" ]]; then
		echo -e "${Error} Tải từ nguồn dự phòng (phiên bản cũ) thất bại!"
		return 1 && exit 1
	else
		tar -xvf "shadowsocks-v1.14.1.${arch}-unknown-linux-gnu.tar.xz"
	fi
	if [[ ! -e "ssserver" ]]; then
		echo -e "${Error} Giải nén từ nguồn dự phòng (phiên bản cũ) thất bại!"
		echo -e "${Error} Cài đặt từ nguồn dự phòng (phiên bản cũ) thất bại!"
		return 1 && exit 1
	else
		...
		echo -e "${Info} Đã tải và cài đặt xong Shadowsocks Rust từ nguồn dự phòng (phiên bản cũ)!"
		return 0
	fi
}
...
Set_tfo(){
	echo -e "Bạn có muốn bật TCP Fast Open không?
==================================
...
=================================="
	...
	echo && echo "=================================="
	echo -e "Trạng thái bật TCP Fast Open: ${Red_background_prefix} ${tfo} ${Font_color_suffix}"
	echo "==================================" && echo
}
...
==================================	
 ...
 ==================================
 ${Tip} Nếu cần phương thức mã hóa khác, vui lòng chỉnh sửa file cấu hình!" && echo
...
Install(){
	...
	Write_config
	echo -e "${Info} Tất cả các bước đã hoàn tất, bắt đầu khởi động..."
	Start
}
...
Uninstall(){
	check_installed_status
	echo "Bạn có chắc chắn muốn gỡ bỏ Shadowsocks Rust? (y/N)"
	echo
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		echo && echo "Đã hủy bỏ gỡ cài đặt..." && echo
	fi
    sleep 3s
    Start_Menu
}
